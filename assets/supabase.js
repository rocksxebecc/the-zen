...
async function fetchNotes() {
  ...
  // Safe query: check if is_private exists or fallback if column is missing
  let { data, error } = await client
    .from('notes')
    .select('id, title, body, updated_at, is_private')
    .order('updated_at', { ascending: false });
    
  if (error && (error.message.includes('is_private') || error.code === '42703')) {
    // Fallback if is_private column is missing
...
async function upsertNote(note) {
  ...
  if (note.hasOwnProperty('is_private')) {
    payload.is_private = note.is_private;
  }
  
  let { error } = await client.from('notes').upsert(payload, { onConflict: 'id' });
  
  if (error && (error.message.includes('is_private') || error.code === '42703')) {
    // Fallback: exclude is_private if column does not exist
    delete payload.is_private;
    const fallback = await client.from('notes').upsert(payload, { onConflict: 'id' });
    error = fallback.error;
  }
...
async function getUserProfile() {
  ...
  // Base profile from auth metadata
  const base = {
    email: user.email,
  ...
  // Enrich with profiles table row (gets is_admin, username, avatar, etc.)
  try {
    const client = getSupabaseClient();
    if (client) {
  ...
  } catch (_) { /* non-fatal — profile table may not exist */ }

  return base;
}

// ── 8. Subscription helpers ──────────────────────────────────

/**
 * checkSubscription()
 * Returns the user's subscription row if active, or null.
 * Caches in sessionStorage for 5 minutes to avoid repeated queries.
 */
async function checkSubscription() {
  const client = getSupabaseClient();
  if (!client) return null;

  // Get current user id explicitly - never rely on RLS alone
  ...
  if (error) { console.error('[Zenspace] checkSubscription error:', error.message); return null; }

  const isActive = data && (!data.expires_at || new Date(data.expires_at) > new Date());
  return isActive ? data : null;
}

/**
 * invalidateSubCache()
 * Call after a successful payment to clear the 5-minute cache.
 */
function invalidateSubCache() {
  // Cache removed — checkSubscription always fetches fresh from DB
}

/**
 * sendWelcomeEmailIfNew()
 * Invokes the send-email edge function if the user hasn't received it yet.
 */
async function sendWelcomeEmailIfNew() {
  ...
  if (profile && !profile.welcome_email_sent) {
    ...
    try {
      ...
      console.error('[Zenspace] Failed to send welcome email:', err);
    }
  }
}
